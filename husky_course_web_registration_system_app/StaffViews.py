...
from .models import CustomUser, Staffs, Courses, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult, Department, Messages, Teacher
...
def staff_home(request):
    ...
    advisor = Staffs.objects.get(admin=request.user.id)
    students_count = Students.objects.filter(
        department_id=advisor.department_id).count()
    teachers = Teacher.objects.filter(department_id=advisor.department_id)
    teacher_count = teachers.count()
    course_count = 0
    for teacher in teachers:
                course_count += Courses.objects.filter(teacher_id=teacher).count()
...
def staff_message(request):
    staff = Staffs.objects.get(admin=request.user.id)
    students = Students.objects.filter(department_id=staff.department_id)
    context = {
        'students': students
    }
    return render(request, "staff_template/staff_message_template.html", context)


def staff_message_save(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method.")
        return redirect('staff_message')
    else:
        try:
            student_id = request.POST.get('student_id')
            content = request.POST.get('content')
            sender_obj = Staffs.objects.get(admin=request.user.id)
            receiver_obj = Students.objects.get(id=student_id)

            add_message = Messages(sender_id=sender_obj,
...
def manage_student_registration(request):
    ...
    students = Students.objects.filter(
        department_id=staff.department_id).all()
    print(f'students type: {type(students)}')
    context = {
        'department': staff.department_id,
        'students': students,
    }
    return render(request, "staff_template/manage_student_registration.html", context)
...
def approve_student_registration(request, student_id):
    ...
    student.save()
    messages.success(
        request, f'Successfully approved {student.admin.first_name}, {student.admin.last_name}\'s registration!')
    return redirect(manage_student_registration)


def decline_student_registration(request, student_id):
    student = Students.objects.get(id=student_id)
    student.is_approved = False
    student.save()
    messages.error(
        request, f'You have declined {student.admin.first_name}, {student.admin.last_name}\'s registration.')
    return redirect(manage_student_registration)
